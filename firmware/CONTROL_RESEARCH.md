...
3. **Accelerometer output contains gravity.** TDK describes acceleration as containing both motion and gravity components. Stationary calibration, a slowly changing baseline and selected-axis monitoring help reject unwanted triggers, but do not solve all tilt/translation ambiguity. [TDK IMU overview](https://product.tdk.com/en/techlibrary/productoverview/imu.html)
...
| Horizontal-only live mode | Tilt, yaw, other-axis movement and clipped readings do not shut down live control. Quiet is determined only from the selected horizontal motion estimate. | Does not cancel torque or correct tilt contamination. Physical motion can exceed sensor range and clip. |
