...
inline bool opposingThroughHorizon(int8_t forceSign, float velocity,
                                  float acceleration, float horizonSeconds,
                                  float minimumSpeed) {
  if (!isfinite(velocity) || !isfinite(acceleration)) return false;
...
struct DirectionGate {
  ...
  void reset() { candidate = 0; confirmed = 0; lastPulsed = 0; candidateSince = 0; }

  int8_t observe(uint32_t now, float velocity, float minimumSpeed,
                 uint16_t confirmMs) {
    int8_t direction = velocity > minimumSpeed ? 1 :
                       (velocity < -minimumSpeed ? -1 : 0);
    if (direction != candidate) { candidate = direction; candidateSince = now; }
    // A brief sign change or dip into the dead band does not earn another pulse.
    if (!direction || now-candidateSince < confirmMs) return 0;
    if (direction != confirmed) { confirmed = direction; lastPulsed = 0; }
    return direction == lastPulsed ? 0 : direction;
  }
  void fired(int8_t direction) { lastPulsed = direction; }
};
}
#endif
