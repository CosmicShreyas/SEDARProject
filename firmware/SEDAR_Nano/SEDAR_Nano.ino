...
// Review pin assignments and hardware polarity here. Times are milliseconds unless marked
// otherwise.
// ============================================================================
...
static_assert((CHANNEL_FORCE_SIGN[0] == -1 && CHANNEL_FORCE_SIGN[1] == 1) ||
                  (CHANNEL_FORCE_SIGN[0] == 1 && CHANNEL_FORCE_SIGN[1] == -1),
              "Signs must be opposite +/-1");
...
struct Sample {
  float a[3];
  float w[3];
}; // acceleration m/s^2; angular rate deg/s.
...
float dynamicA = 0, velocityEstimate = 0;
...
// Release both relay outputs and account for the completed pulse.
void outputsOff() {
  writeRelay(0, false);
  writeRelay(1, false);
  if (activeChannel >= 0) {
    ...
    if (onMs > HARD_MAX_ON_MS)
      onMs = HARD_MAX_ON_MS; // Ignore rollover artefacts.
...
// Explicit STOP/fault handling clears automatic arming; ordinary movement does not.
void disarm() {
  outputsOff();
  armed = false;
  episode = false;
...
void servicePulse(uint32_t now) {
  ...
  if (age >= PULSE_MS || age >= HARD_MAX_ON_MS || faulted || !sensorReady || !calibrated ||
      (!manualPulse && !armed))
    outputsOff();
}
// ============================================================================
...
bool readRegisters(uint8_t reg, uint8_t count, uint8_t *out) {
  ...
  if (Wire.endTransmission(false) != 0)
    return false;
  if (Wire.requestFrom(mpuAddress, count, (uint8_t)true) != count)
    return false;
...
bool initSensor() {
  ...
  for (uint8_t tries = 0; tries < 20 && !cleared; ++tries) {
    uint8_t power = 0;
    if (readRegisters(0x6B, 1, &power) && !(power & 0x80))
      cleared = true;
    else
      delay(10);
  }
  ...
  uint8_t power = 0;
  if (!readRegisters(0x6B, 1, &power) || (power & 0x40))
    return false;
  ...
  if (!writeRegister(0x6C, 0x00) || // All axes awake.
    ...
    return false; // Accel +/-4 g: 8192 LSB/g.
  initStage = F("read back config 0x19..0x1C");
  uint8_t settings[4];
  if (!readRegisters(0x19, 4, settings))
    return false;
  initStage = F("config read back wrong values");
  if (!(settings[0] == 9 && settings[1] == 3 && settings[2] == 8 && settings[3] == 8))
    return false;
  initStage = F("ok");
  return true;
}
...
// Read all six channels for calibration; normal feedback uses only MOTION_AXIS.
bool readSample(Sample &s) {
  uint8_t b[14];
  if (!readRegisters(0x3B, 14, b))
    return false;
  for (uint8_t i = 0; i < 3; ++i) {
    s.a[i] = signedWord(b + 2 * i) * (G / 8192.0f);
    s.w[i] = signedWord(b + 8 + 2 * i) / 65.5f;
    // Reaching a measurement limit during movement is not a communication fault.
    // Readings may clip, but movement alone must not disarm the demonstration.
  }
  return true;
}
// ============================================================================
...
void beginCalibration() {
  ...
  if (!sensorReady) {
    Serial.print(F("MPU6050 init failed at stage: "));
    Serial.println(initStage);
    Serial.print(F("  i2cAddr=0x"));
    Serial.print(mpuAddress, 16);
    Serial.print(F(" whoAmI=0x"));
    Serial.println(whoAmI, 16);
...
void calibrateSample(const Sample &s) {
  ++calCount;
  for (uint8_t i = 0; i < 3; ++i) {
    float da = s.a[i] - meanA[i];
    meanA[i] += da / calCount;
    m2A[i] += da * (s.a[i] - meanA[i]);
    float dw = s.w[i] - meanW[i];
    meanW[i] += dw / calCount;
  ...
  for (uint8_t i = 0; i < 3; ++i) {
    ...
    if (i == MOTION_AXIS)
      measuredAccelSigma = sqrtf(varA);
    if (varA > 0.04f || varW > 1.0f || fabs(gyroBias[i]) > 10) {
      fault(F("Moved during calibration or excessive gyro bias"));
      return;
    }
    gravity[i] = baselineA[i];
  }
  float norm =
      sqrt(baselineA[0] * baselineA[0] + baselineA[1] * baselineA[1] + baselineA[2] * baselineA[2]);
  if (norm < 0.85f * G || norm > 1.15f * G || fabs(baselineA[0]) > 0.20f * G ||
      fabs(baselineA[1]) > 0.20f * G || baselineA[2] < 0.8f * G) {
    fault(F("Mount board flat, +Z up; check accelerometer readings"));
    return;
  }
...
void serviceThermal(uint32_t now) {
  ...
  if (elapsed < 100)
    return; // Coarse steps keep the integer maths exact.
  uint32_t bleed = (elapsed * THERMAL_BUDGET_MS) / THERMAL_WINDOW_MS;
  if (!bleed)
    return; // Wait for a whole millisecond of credit.
...
bool startPulse(uint8_t channel, bool manual, uint32_t now) {
  if (channel > 1 || activeChannel >= 0 || faulted || !sensorReady || !calibrated)
    return false;
  ...
  if (thermalOnMs[channel] + PULSE_MS > THERMAL_BUDGET_MS)
    return false;
  if (!manual && (!ENABLE_EXPERIMENTAL_CONTROL || !armed))
    return false;
  if (now - lastOff < COOLDOWN_MS)
    return false;
  ...
  while (historyCount && now - pulseHistory[historyHead] >= WINDOW_MS) {
    historyHead = (historyHead + 1) % MAX_PULSES_PER_WINDOW;
    --historyCount;
  }
  if (historyCount >= MAX_PULSES_PER_WINDOW)
    return false;
  ...
  pulseStarted = now;
  manualPulse = manual;
  writeRelay(channel, true);
  return true;
}
// ============================================================================
// 08 / FILTERING, MOTION ESTIMATION AND OPTIONAL FEEDBACK
// Every calibrated sample updates the selected horizontal motion estimate. The default
...
void controlSample(const Sample &s, float dt, uint32_t now) {
  ...
  gravity[MOTION_AXIS] += (dt / (0.75f + dt)) * (s.a[MOTION_AXIS] - gravity[MOTION_AXIS]);
  ...
  // Live control considers only the configured horizontal axis (X by default).
  // Vertical movement, roll, pitch, yaw and measurement clipping do not fault,
  // disarm or inhibit this controller. Communication/configuration faults remain.
  bool quiet = fabs(dynamicA) < accelQuietThreshold && fabs(velocityEstimate) < VELOCITY_RELEASE;
  if (quiet) {
    if (!quietTracking) {
      quietTracking = true;
      quietStarted = now;
    ...
    quietTracking = false;
  if (!ENABLE_EXPERIMENTAL_CONTROL || !armed)
    return;
  ...
  if (activeChannel >= 0 && !manualPulse &&
      !Sedar::opposingThroughHorizon(CHANNEL_FORCE_SIGN[activeChannel], velocityEstimate, dynamicA,
  ...
  const float fullHorizon =
      (ACTUATION_DELAY_MS + PULSE_MS + FORCE_RELEASE_MS + TIMING_MARGIN_MS) * 0.001f;
  ...
  if (!Sedar::opposingThroughHorizon(wantedForce, velocityEstimate, dynamicA, fullHorizon,
                                     VELOCITY_RELEASE))
    return;
  for (uint8_t i = 0; i < 2; ++i)
...
// ============================================================================
// Print configuration and state without changing the existing arming flag.
void printStatus() {
  ...
  Serial.print(armed);
  Serial.print(F(" axis="));
  Serial.print(MOTION_AXIS == 0 ? 'X' : 'Y');
  ...
  Serial.print(F(" whoAmI=0x"));
  Serial.println(whoAmI, 16);
  Serial.print(F("experimentalControl="));
  ...
  Serial.println(accelQuietThreshold, 3);
  // Remaining coil ON-time allowance per channel before pulses are refused.
  Serial.print(F("thermalUsedMs ch1="));
  Serial.print(thermalOnMs[0]);
  Serial.print(F(" ch2="));
...
void executeCommand() {
  ...
  if (!strcmp(commandBuffer, "STOP")) {
    ...
    if (!ENABLE_EXPERIMENTAL_CONTROL) {
      disarm();
      Serial.println(F("ARM locked: experimental feedback not validated. See REALITY_CHECK.md."));
    } else if (faulted || !calibrated || !sensorReady || !quietTracking ||
               millis() - quietStarted < QUIET_MS)
      Serial.println(F("ARM refused: calibrate and keep still first."));
    else {
      armed = true;
      episode = false;
      waitingForQuiet = false;
    ...
    if (armed)
      Serial.println(F("Send STOP before individual tests."));
    else
      Serial.println(startPulse(commandBuffer[4] - '1', true, millis())
                         ? F("Test pulse started.")
                         : F("Test refused: calibration/fault/cooldown/budget."));
  } else if (!strcmp(commandBuffer, "STREAM ON")) {
    streaming = true;
    Serial.println(F("a_mm_s2,v_mm_s,ch,armed"));
  } else if (!strcmp(commandBuffer, "STREAM OFF"))
    streaming = false;
    ...
    if (!seen) {
      ...
      Serial.print(F("  probe 0x68 returned "));
      Serial.println(code);
...
void serviceSerial() {
  ...
  for (uint8_t n = 0; n < 16 && Serial.available(); ++n) {
    ...
    if (c == '\r' || c == '\n') {
      if (commandOverflow) {
        disarm();
        Serial.println(F("Command too long; disarmed."));
...
void loop() {
  ...
  if (sensorReady && !faulted && (int32_t)(us - nextSample) >= 0) {
    ...
    if (elapsed > 50000UL) {
      fault(F("Sampling stalled for over 50 ms"));
      return;
    }
    ...
    if (millis() - lastConfigCheck >= 250) {
      ...
      uint8_t cfg[4], power;
      if (!readRegisters(0x19, 4, cfg) || !readRegisters(0x6B, 1, &power) || cfg[0] != 9 ||
          cfg[1] != 3 || cfg[2] != 8 || cfg[3] != 8 || power != 1) {
        fault(F("Sensor configuration changed or read failed"));
        return;
      }
    }
    // Do not integrate a repeated output register as a fresh measurement.
    uint8_t ready;
    if (!readRegisters(0x3A, 1, &ready)) {
      fault(F("Sensor status read failed"));
      return;
    }
    if (!(ready & 0x01)) {
      servicePulse(millis());
    ...
    if (calibrating)
      ...
      controlSample(sample, elapsed * 0.000001f, millis());
  }
  ...
  if (streaming && calibrated && millis() - lastReport >= 100 && Serial.availableForWrite() >= 48) {
    lastReport = millis();
    Serial.print((long)(dynamicA * 1000));
    Serial.print(',');
    Serial.print((int)(velocityEstimate * 1000));
    Serial.print(',');
